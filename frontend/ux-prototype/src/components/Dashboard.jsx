...
const StatCard = ({ icon: Icon, label, value, trend, color }) => (
  <div className="glass-morphism p-6 rounded-2xl flex flex-col gap-2">
    <div className="flex items-center justify-between">
      ...
      <span
        className={`text-xs font-semibold px-2 py-1 rounded bg-green-500/10 text-green-400`}
      >
        {trend}
      </span>
    </div>
...
const Dashboard = () => {
  const recentDocs = [
    ...
    {
      id: 2,
      name: "contrato_servicios_v2.docx",
      status: "En cola",
      time: "hace 15 min",
      type: "Legal",
    },
    {
      id: 3,
      name: "infraestructura_enclave.yaml",
      status: "Error",
      time: "hace 1 h",
      type: "GitOps",
    },
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Bienvenido de nuevo. Aquí tienes un resumen de la actividad local.
          </p>
        </div>
        ...
        <div className="lg:col-span-2 glass-morphism rounded-3xl p-8 h-[400px] flex flex-col">
          ...
          <div className="flex-1 flex items-end gap-4 px-4 overflow-hidden">
            {[60, 45, 80, 55, 90, 70, 85].map((h, i) => (
              <div
                key={i}
                className="flex-1 bg-gradient-to-t from-blue-600 to-indigo-400 rounded-t-lg transition-all duration-1000"
                style={{ height: `${h}%` }}
              ></div>
            ))}
          </div>
        ...
        <div className="glass-morphism rounded-3xl p-8">
          ...
          <div className="space-y-4">
            {recentDocs.map((doc) => (
              <div
                key={doc.id}
                className="flex items-center justify-between p-4 rounded-xl bg-slate-800/40 hover:bg-slate-800 transition-colors border border-slate-700/50"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-slate-200 truncate max-w-[120px]">
                      {doc.name}
                    </h4>
                ...
                <span
                  className={`text-[10px] font-bold px-2 py-1 rounded tracking-wider uppercase ${
                    doc.status === "Procesado"
